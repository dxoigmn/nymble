...
void user_free(user_t *user) {
  ...
  while (user_entry) {
    ...
    //printf("free(%x,user_entry)\n", (u_int)user_entry);
    free(user_entry);
    
    user_entry = next;
  }
  
  //printf("free(%x,user->verify_key_n)\n", (u_int)user->verify_key_n);
...
u_int user_blacklist_update(user_t *user, u_char *server_id, blacklist_t *blacklist, u_int link_window, u_int time_period) {
  if (!bl_check_integrity(blacklist, server_id, user->verify_key_n) ||
    ...
    //printf("returning early %d %d\n", bl_check_integrity(blacklist, server_id, user->verify_key_n), bl_check_freshness(blacklist, time_period, link_window));
    return 0;
  }
  ...
  if (user_entry == NULL) {
    //printf("returning early because no user\n");
    return 0;
  }
